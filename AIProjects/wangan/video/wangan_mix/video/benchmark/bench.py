...
|测试名称|测试数据集|并发|检查|倍速|响应时间(95%)|(90%)|max|min|med|arg|
|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|
...
def parse_k6_data(k6_output_text):
    ...
    for txt_line in txt_lines:
        if 'checks..' in txt_line:
            ...
            result['iterations'] = str(float(re.search(r'(\S+/s)$', txt_line).group(1)[:-2])*60.0)
...
{k6js_path} | tee {out_txt_path}'''
    run(cmd.format(
        ...
        interval=benchmark_args['interval'],
...
def run_all(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    try:
        for benchmark_set in config['benchmarks']:
            ...
            interval = benchmark_set['interval']
            for tsv in tsv_list:
                for vus in vus_list:
