...
typedef struct
{
	S_Dev_Info*   pLatestDevArray[MAX_SIZE_FOR_ONVIF_DESC];
	int           iMaxLatestDevCount;
	int           iCurLatestDevCount;

	pthread_mutex_t    tMutex;
	char          strIP[MAX_SIZE_FOR_IP];
	char*         pStrJson;
	int           iBufSizeForJson;
	int           iThreadRunFlag;
} S_Dev_Discover_Info;
...
void  GlobalInit();
int InitDevDiscoverInfo(S_Dev_Discover_Info*  pDevDiscoer);
void UnInitDevDiscoverInfo(S_Dev_Discover_Info*  pDevDiscoer);
