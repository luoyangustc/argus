...
#define CURRENT_SERVER_NAME "stream_serv"
...
static void SetLoglevel(int log_level)
{
    ...
    {
        string log_file = CConfigHelper::get_default_config_dir() + "/log/librtmp.log";
        static FILE* pFile = fopen(log_file.c_str(), "ab");
        RTMP_LogSetOutput(pFile);
...
static int StartService()
{
    string cfg_file = CConfigHelper::get_default_config_filename();
    if( !CServerLogical::GetLogical()->InitCfg(cfg_file) )
    {
        return -1;
    }

    int log_level = CServerLogical::GetLogical()->GetServCfg()->GetLogLevel();
    uint16 http_port = CServerLogical::GetLogical()->GetServCfg()->GetHttpPort();
    uint16 serv_port = CServerLogical::GetLogical()->GetServCfg()->GetServPort();
    string log_path = CConfigHelper::get_default_config_dir() + "/log";

    SetLoglevel(log_level);

    if ( ! CServerLogical::GetLogical()->Start() )
    {
