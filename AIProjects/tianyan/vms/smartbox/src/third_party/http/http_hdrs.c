...
http_hdr_is_known(const char *a_hdr)
{
	...
	{
		 if (vos_ansi_strcasecmp(a_hdr, http_hdr_known_list[l_pos]) == 0)
		{
...
http_hdr_list_new(void)
{
  ...
  l_return = VOS_MALLOC_T(http_hdr_list);//(http_hdr_list *)malloc(sizeof(http_hdr_list));
...
http_hdr_list_destroy(http_hdr_list *a_list)
{
  ...
  for(i=0; i < HTTP_HDRS_MAX; i++)
	...
	VOS_FREE_T(a_list->header[i]);
      if (a_list->value[i])
	VOS_FREE_T(a_list->value[i]);
    }
  VOS_FREE_T (a_list);
}
			  ...
			  int a_val_len)
{
  ...
  if ((a_list == NULL) ||
      ...
      (a_name_len == 0))
    goto ec;
  l_temp_name = VOS_MALLOC_BLK_T(char, a_name_len + 1);//(char *)malloc(a_name_len + 1);
  memset(l_temp_name, 0, a_name_len + 1);
  memcpy(l_temp_name, a_name_start, a_name_len);
  l_temp_val = VOS_MALLOC_BLK_T(char, a_val_len + 1);//(char *)malloc(a_val_len + 1);
  ...
  l_return = http_hdr_set_value(a_list,
				l_temp_name,
				l_temp_val);
  VOS_FREE_T(l_temp_name);
  VOS_FREE_T(l_temp_val);
 ec:
  return l_return;
  ...
  if (l_temp_value == NULL)
      ...
      for (i=0; i < HTTP_HDRS_MAX; i++)
	{
	  if (a_list->header[i] == NULL)
		...
		a_list->header[i] = vos_strdup(a_name);
	      a_list->value[i] = vos_strdup(a_val);
	      l_return = 1;
	      break;
	    }
	}
		   ...
		   const char *a_name)
{
  ...
  for (i=0; i < HTTP_HDRS_MAX; i++)
    {
      if (a_list->header[i] &&
	  (vos_ansi_strcasecmp(a_list->header[i], a_name) == 0))
	{
		     ...
		     int *a_num_names)
{
  ...
  *a_num_names = 0;
  ...
  /* now that we know how many headers we have allocate the number of
     slots in the return */
  l_names = VOS_MALLOC_BLK_T(char*, l_num_names);//malloc(sizeof(char *) * l_num_names);
  if (l_names == NULL)
    return -1;
  ...
  for (i=0; i < HTTP_HDRS_MAX; i++)
    {
      if (a_list->header[i])
	{
	  l_names[i] = vos_strdup(a_list->header[i]);
	  if (l_names[i] == NULL)
	    goto ec;
	}
    }
