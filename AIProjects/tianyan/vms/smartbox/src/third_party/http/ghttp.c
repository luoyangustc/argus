...
ghttp_request_new(void)
{
  ...
  /* create everything */
  l_return = VOS_MALLOC_T(struct _ghttp_request);
...
ghttp_request_destroy(ghttp_request *a_request)
{
  ...
  if (a_request->conn->sock >= 0)
    {
		vos_sock_close(a_request->conn->sock);
  ...
  if (a_request->uri)
	  ...
	  a_request->uri->resource)
	{
	  ...
	  if ((!strcmp(a_request->uri->host, l_new_uri->host)) &&
	    ...
	    {
	      VOS_FREE_T(a_request->uri->resource);
	      /* make a copy since we're about to destroy it */
	      a_request->uri->resource = vos_strdup(l_new_uri->resource);
	      http_uri_destroy(l_new_uri);
	    }
	  else
	    {
		   ...
		   const char *a_pass)
{
  ...
  */
  ...
  l_authtoken = VOS_MALLOC_BLK_T(char, strlen(a_user) + strlen(a_pass) + 2);
  ...
  if (!l_auth64)
    {
      VOS_FREE_T(l_authtoken);
      return -1;
    }
  /* build the final header */
  l_final_auth = VOS_MALLOC_BLK_T(char, strlen(l_auth64) + strlen(basic_header) + 1);
  ...
  if (a_request->username) VOS_FREE_T(a_request->username);
  if (a_request->password) VOS_FREE_T(a_request->password);
  if (a_request->authtoken) VOS_FREE_T(a_request->authtoken);
  a_request->username = vos_strdup(a_user);
  a_request->password = vos_strdup(a_pass);
			 ...
			 const char *a_pass)
{
  ...
  */
  ...
  VOS_FREE_T(l_authtoken);
  /* copy the strings into the request */
  if (a_request->proxy_username) VOS_FREE_T(a_request->proxy_username);
  if (a_request->proxy_password) VOS_FREE_T(a_request->proxy_password);
  if (a_request->proxy_authtoken) VOS_FREE_T(a_request->proxy_authtoken);
  a_request->proxy_username = vos_strdup(a_user);
  a_request->proxy_password = vos_strdup(a_pass);
