...
http_req_new(void)
{
  ...
  l_return = VOS_MALLOC_T(http_req);//(http_req *)malloc(sizeof(http_req));
...
http_req_destroy(http_req *a_req)
{
  ...
  //VOS_PRINTF("http_req_destroy--->free a_req\n");
  VOS_FREE_T(a_req);
}
...
http_req_send(http_req *a_req, http_trans_conn *a_conn)
{
  ...
  l_request = VOS_MALLOC_BLK_T(char, 30 + strlen(a_req->resource) + (a_conn->proxy_host ?
						        (strlen(a_req->host) + 20) : 0));
