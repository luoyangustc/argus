...
int http_trans_connect(http_trans_conn *a_conn)
{
    ...
    if (a_conn->hostinfo == NULL)
    {
        ...
        if (a_conn->proxy_host)
        {
            if ((a_conn->hostinfo = gethostbyname(a_conn->proxy_host)) == NULL)
            {
                ...
                a_conn->errstr = (char*)http_trans_get_host_error(a_conn->error);

                VOS_PRINTF("get host by name fail, prox_host=%s, error=%d, errstr=%s\n", a_conn->proxy_host, a_conn->error, a_conn->errstr);
                goto ec;
            }
        }
        else
        {
            ...
            if ((a_conn->hostinfo = gethostbyname(a_conn->host)) == NULL)
            {
                ...
                VOS_PRINTF("get host by name fail, host=%s, error=%d, errstr=%s\n", a_conn->host, a_conn->error, a_conn->errstr);
                goto ec;
            }
        }
        /* set up the saddr */
        ...
        else
        {
            ...
            //a_conn->saddr.sin_port = htons(1988);
        }
        /* copy the name info */
#if (OS_WIN32 == 1)
        memcpy(&a_conn->saddr.sin_addr.S_un.S_addr, a_conn->hostinfo->h_addr_list[0], sizeof(unsigned long));
#else
    ...
    if ((a_conn->sock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
    {
        ...
        a_conn->error = vos_get_native_netos_error();
        VOS_PRINTF("create socket fail, error=%d\n", a_conn->error);
        goto ec;
    }
    //VOS_PRINTF("create socket ok, sock=%d, port = %d\n", a_conn->sock, a_conn->port);
...
http_trans_conn* http_trans_conn_new(void)
{
  ...
  l_return =VOS_MALLOC_T(http_trans_conn);//(http_trans_conn *)malloc(sizeof(http_trans_conn));
  ...
  l_return->io_buf = VOS_MALLOC_BLK_T(char, l_return->io_buf_chunksize);//malloc(l_return->io_buf_chunksize);
...
void http_trans_conn_destroy(http_trans_conn *a_conn)
{
    ...
    if (a_conn->io_buf)
    {
        VOS_FREE_T(a_conn->io_buf);//free(a_conn->io_buf);
    }
...
int http_trans_append_data_to_buf(http_trans_conn *a_conn,
			      char *a_data,
			      int   a_data_len)
{
    if (http_trans_buf_free(a_conn) < a_data_len)
    {
        ...
        char* new_io_buf = VOS_MALLOC_BLK_T(char, a_conn->io_buf_len + a_data_len);
        if( !new_io_buf )
        {
            return HTTP_TRANS_ERR;
        }
        memcpy(new_io_buf, a_conn->io_buf, a_conn->io_buf_len);
        VOS_FREE_T(a_conn->io_buf);
        a_conn->io_buf = new_io_buf;
        a_conn->io_buf_len += a_data_len;
    }
...
int http_trans_read_into_buf(http_trans_conn *a_conn)
{
    ...
    if (http_trans_buf_free(a_conn) < a_conn->io_buf_io_left)
    {
        ...
        char* new_io_buf = VOS_MALLOC_BLK_T(char, a_conn->io_buf_len + a_conn->io_buf_io_left);
        if( !new_io_buf )
        {
            return HTTP_TRANS_ERR;
        }
    ...
    if ((a_conn->last_read = l_read = recv(a_conn->sock,
                     ...
                     l_bytes_to_read, 0)) < 0)
    {
        if (vos_get_native_netos_error() == VOS_EINTR)
            l_read = 0;
        else
            return HTTP_TRANS_ERR;
    }
...
void http_trans_buf_reset(http_trans_conn *a_conn)
{
    ...
    a_conn->io_buf = VOS_MALLOC_BLK_T(char, a_conn->io_buf_chunksize);//malloc(a_conn->io_buf_chunksize);
