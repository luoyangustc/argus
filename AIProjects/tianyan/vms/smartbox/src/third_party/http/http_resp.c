...
static int string_is_number(char *a_string);

static int read_chunk(http_trans_conn *a_conn);

static int read_body_chunked(http_resp *a_resp,
		  http_req *a_req,
		  http_trans_conn *a_conn);

static int read_body_content_length(http_resp *a_resp,
			 http_req *a_req,
			 http_trans_conn *a_conn);

static int read_body_standard(http_resp *a_resp,
		   http_req *a_req,
		   http_trans_conn *a_conn);

http_resp* http_resp_new(void)
{
  http_resp *l_return = NULL;

  l_return = VOS_MALLOC_T(http_resp);//(http_resp *)malloc(sizeof(http_resp));
  memset(l_return, 0, sizeof(http_resp));
  l_return->headers = http_hdr_list_new();
  return l_return;
}

void http_resp_destroy(http_resp *a_resp)
{
    if (!a_resp)
        return;
        
    if (a_resp->reason_phrase)
        VOS_FREE_T(a_resp->reason_phrase);
        
    if (a_resp->headers)
        http_hdr_list_destroy(a_resp->headers);
        
    if (a_resp->body)
        VOS_FREE_T(a_resp->body);
...
int http_resp_read_headers(http_resp *a_resp, http_trans_conn *a_conn)
{
  ...
  header_state   l_state = reading_header;
    ...
    a_resp->reason_phrase =
        VOS_MALLOC_BLK_T(char, (l_cur_ptr - (&l_ptr[13])) + 1);//malloc((l_cur_ptr - (&l_ptr[13])) + 1);
		    ...
		    http_trans_conn *a_conn)
{
  ...
  if (l_content_length && (a_req->type != http_req_type_head))
  ...
  else if (l_transfer_encoding)
    {
      ...
      if (!vos_ansi_strcasecmp(l_transfer_encoding, "chunked"))
	{
	  /* set the state */
  ...
  if (l_connection && (l_return != HTTP_TRANS_NOT_DONE))
    {
		if (!vos_ansi_strcasecmp(l_connection, "close"))
		{
		  //close (a_conn->sock);
			vos_sock_close(a_conn->sock);
		  a_conn->sock = -1;
		  ...
		  http_trans_conn *a_conn)
{
  ...
  do
      ...
      if (a_conn->chunk_len > 0)
	{
	  if (a_resp->body == NULL)
	    {
	      a_resp->body = VOS_MALLOC_BLK_T(char, a_conn->chunk_len+1);//malloc(a_conn->chunk_len);
	      memcpy(a_resp->body, a_conn->io_buf, a_conn->chunk_len);
	      a_resp->body[a_conn->chunk_len] = '\0';
	  ...
	  /* append it to the body */
	  else
	    {
	      //a_resp->body = realloc(a_resp->body,
			//	     (a_resp->body_len + a_conn->chunk_len));
		  char* new_io_buf = VOS_MALLOC_BLK_T(char, a_resp->body_len + a_conn->chunk_len+1);
          if( !new_io_buf )
          {
            return HTTP_TRANS_ERR;
          }
          memcpy(new_io_buf, a_resp->body, a_resp->body_len);
          VOS_FREE_T(a_resp->body);
          a_conn->io_buf = new_io_buf;
          
	      memcpy(&a_resp->body[a_resp->body_len], a_conn->io_buf, a_conn->chunk_len);
	      a_resp->body[a_resp->body_len + a_conn->chunk_len] = '\0';
	      a_resp->body_len += a_conn->chunk_len;
	    }
	}
                    ...
                    http_trans_conn *a_conn)
{
  ...
  a_resp->body = VOS_MALLOC_BLK_T(char, a_conn->io_buf_alloc + 1);//malloc(a_conn->io_buf_alloc + 1);
