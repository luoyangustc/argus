...
//float *p = &classification_output.scores[0];
//std::cout << *p  << std::endl;

const void* features_bytes = reinterpret_cast<const void*>(&classification_output.scores[0]);
int size_classification = (classification_output.scores.size())*4;
res->set_body(features_bytes,size_classification);

//std::cout << "classification dim" << (classification_output.scores.size()) << std::endl;
          res->set_code(status);
