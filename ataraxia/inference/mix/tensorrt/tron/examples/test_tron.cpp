...
int main(int argc, char const *argv[]) {
  if(argc != 6){
    ...
    std::cout<<"./build/tron/test_tron models/mix_engin.bin models/onet.bin models/face-feature-res18.bin CLS-MIX-DET-test-10000-images/filelist.txt 16"<<std::endl;
    return -1;
  }

  std::cout<<"start testing..."<<std::endl;
  std::string mix_model_file(argv[1]);
  std::string onet_model_file(argv[2]);
  std::string face_feature_model_file(argv[3]);
  ...
  getModel(models, modelSize, 0, mix_model_file.c_str());
  getModel(models, modelSize, 1, onet_model_file.c_str());
  getModel(models, modelSize, 2, face_feature_model_file.c_str());

  inference::CreateParams create_params;
  auto* model_files = create_params.add_model_files();
  model_files->set_name(mix_model_file.c_str());
  ...
  model_files = create_params.add_model_files();
  model_files->set_name(face_feature_model_file.c_str());
  ...
  std::cout<<"Total images: "<<im_list.size()<<" error: "<<code<<std::endl;
  int currCount = 0;
  if (code == 200) {
