...
def copy_weights(arg_params, aux_params, param_dict, tron_net):
    ...
    for tron_op in net_param.op:
        ...
        assert(len(param_names) == len(tron_op.blobs))
        for index in range(0, len(param_names)):
            num_data = 1
            for dim in tron_op.blobs[index].shape:
                num_data *= dim
            ...
            tron_op.blobs[index].data_f.extend(param_data)
...
def find_inputs(json_nodes, json_name, json_inputs):
    ...
    for input_index in json_inputs:
        ...
        if node_op == 'Dropout' or node_op == 'Activation' or node_op == 'Flatten' or node_op == '_copy':
...
def convert_input(net_info, tron_net):
    tron_inputs = net_info['input_name']
    ...
    if num_mean > 0 or num_scale > 0:
        ...
        tron_net.add_data('data_transform', [data_blob_name], [data_blob_name], mean_value, scale_value)


def convert_activate(mxnet_nodes, index, param_dict, tron_net):
    ...
    tron_net.add_activate(json_name, bottom_names, bottom_names, act_type)


def convert_prelu(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    if len(param_names) > 0:
        ...
        param_dict[json_name] = [gamma_name]
    ...
    tron_net.add_activate(json_name, bottom_names, [json_name], act_type)


def convert_batch_norm(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_batch_norm(json_name, bottom_names, [json_name], use_global_stats, eps)
    tron_net.add_scale(json_name + '_scale', [json_name], [json_name], 1, 1, scale_bias_term)


def convert_concat(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_concat(json_name, bottom_names, [json_name], dim)


def convert_connected(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_connected(json_name, bottom_names, [json_name], num_output, bias_term)


def convert_conv(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_conv(json_name, bottom_names, [json_name], num_output, kernel_size[0], stride[0], pad[0], dilate[0], bias_term, group)


def convert_deformable_conv(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_deformable_conv(json_name, bottom_names, [json_name], num_output, kernel_size[0], stride[0], pad[0], dilate[0], bias_term, group, deformable_group)


def convert_deformable_psroi_pooling(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_deformable_psroi_pooling(json_name, bottom_names, [json_name], output_dim, group_size, pooled_size, part_size, sample_per_part, spatial_scale, trans_std, no_trans)


def convert_eltwise(mxnet_nodes, index, param_dict, tron_net, operation):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_eltwise(json_name, bottom_names, [json_name], operation)


def convert_mul_scalar(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_binary(json_name, bottom_names, [json_name], 'Mul', scalar)


def convert_minus_scalar(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_binary(json_name, bottom_names, [json_name], 'Sub', scalar)


def convert_pooling(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_pooling(json_name, bottom_names, [json_name], pool, kernel_size[0], stride[0], pad[0], global_pooling, full_pooling)


def convert_proposal(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_proposal(json_name, bottom_names, [json_name], feat_stride, pre_nms_top_n, post_nms_top_n, min_size, nms_thresh, ratios, scales)


def convert_reshape(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_reshape(json_name, bottom_names, [json_name], shape)


def convert_roi_pooling(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_roi_pooling(json_name, bottom_names, [json_name], pooled_size[0], pooled_size[1], spatial_scale)


def convert_softmax(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_softmax(json_name, bottom_names, [json_name])


def mxnet2tron(model_root, meta_net_info, copy_params=False):
    ...
    for n in range(0, len(mxnet_symbols)):
        ...
        tron_net.set_net(n)
        tron_net.set_net_name(meta_net_info['save_name'] + '_' + str(n))
        tron_net.set_net_num_class(net_info['num_class'])
        tron_net.set_net_arg(net_info['arg'])
        tron_net.set_net_out_blob(find_nodes(mxnet_nodes, mxnet_heads))

        convert_input(net_info, tron_net)
        ...
        for index, json_node in enumerate(mxnet_nodes):
            ...
            if json_op == 'Activation':
                convert_activate(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'LeakyReLU':
                print(json_op)
                convert_prelu(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'BatchNorm':
                convert_batch_norm(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'Concat':
                convert_concat(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'FullyConnected':
                convert_connected(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'Convolution':
                convert_conv(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == '_contrib_DeformableConvolution':
                convert_deformable_conv(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == '_contrib_DeformablePSROIPooling':
                convert_deformable_psroi_pooling(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'elemwise_add' or json_op == 'broadcast_add':
                convert_eltwise(mxnet_nodes, index, param_dict, tron_net, 'Sum')
            elif json_op == '_mul_scalar':
                convert_mul_scalar(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == '_minus_scalar':
                convert_minus_scalar(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'Pooling':
                convert_pooling(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == '_contrib_Proposal':
                convert_proposal(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'Reshape':
                convert_reshape(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'ROIPooling':
                convert_roi_pooling(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'SoftmaxOutput' or json_op == 'SoftmaxActivation' or json_op == 'softmax':
                convert_softmax(mxnet_nodes, index, param_dict, tron_net)
            else:
                print('Skipping ' + json_op, ' please check!')

        if copy_params:
            copy_weights(mxnet_arg_params[n], mxnet_aux_params[n], param_dict, tron_net)
