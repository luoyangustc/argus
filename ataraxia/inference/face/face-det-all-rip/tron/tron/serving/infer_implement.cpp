...
namespace Tron {
    ...
    void ShadowDetectionRefineDet::Predict( const cv::Mat &im_mat, const VecRectF &rois, std::vector<VecBoxF> *Gboxes,
        ...
        for (int b = 0; b < Gboxes->size(); ++b) {
            ...
            // sort(Gboxes->at(b).begin(), Gboxes->at(b).end(), AreaCompare);
        }
    }
#endif
    ...
    void QualityEvalutation::Predict(const cv::Mat &im_mat, const VecBoxF &face_boxes,
        ...
        if(qa_boxes.size()>0) {
            ...
            Process(in_data_, qa_boxes,Qaprob,Qalabels);
        }
    ...
    void QualityEvalutation::Process(const VecFloat &in_data, const VecBoxF &qa_boxes, std::vector<std::vector<float>>& Qaprob,
        ...
        for(int round=0;round<rounds;round++){
            ...
            for(int iter=0;iter<curr_batch;iter++){
                ...
                const auto* cls_ptr = quality_cls_data+iter*cls_dim_;//5 categories
                // const auto* ori_ptr = orient_cls_data+iter*ori_dim_;//8 categories
    ...
    /************************* face orientation regression **************************************/
    void OrientationRegression::Setup(const tron::MetaNetParam &meta_net_param,
                                   const VecInt &in_shape, int gpu_id) {
        net_.Setup(gpu_id);
        net_.LoadModel(meta_net_param.network(0));
        data_shape_ = net_.GetBlobByName<float>("data")->shape();
        ...
        const auto &out_blob = net_.out_blob();
        CHECK_EQ(out_blob.size(), 2);
        prob_updown_  = net_.out_blob()[0];
        orient_value_ = net_.out_blob()[1];
    ...
    void OrientationRegression::Predict(const cv::Mat &im_mat, const VecBoxF &face_boxes,
                                     std::vector<float>& Orientation, std::vector<int> Orientation_filter,
                                     int min_face){
        ...
        for (const auto &box : face_boxes){
            ++index;
            //filter Orientation_filter[index]
            if (Orientation_filter[index]!=1){
                continue;
            }
            float center_x = (box.xmin + box.xmax)/2;
            float center_y = (box.ymin + box.ymax)/2;
            float rec_width = std::max(box.xmax-box.xmin, box.ymax-box.ymin) / 2;
            BoxF exbox;
            exbox.xmin=std::max(0,cvRound(center_x-rec_width));
            exbox.ymin=std::max(0,cvRound(center_y-rec_width));
            exbox.xmax=std::min(im_mat.cols-1,cvRound(center_x+rec_width));
            exbox.ymax=std::min(im_mat.rows-1,cvRound(center_y+rec_width));
            or_boxes.push_back(exbox);
        }
        
        if(or_boxes.size()>0) {
            in_data_.resize(or_boxes.size() * net_in_num_);
            for (int n = 0; n < or_boxes.size(); ++n) {
                const auto &face_box = or_boxes[n];
                ConvertData(im_mat, in_data_.data() + n * net_in_num_,
                            face_box.RectFloat(), net_in_c_, net_in_h_, net_in_w_);
            }

            Process(in_data_, or_boxes, Orientation, Orientation_filter);
        }
    }

    void OrientationRegression::Release() { net_.Release(); }

    void OrientationRegression::Process(const VecFloat &in_data, const VecBoxF &or_boxes,
                                        std::vector<float>& Orientation, std::vector<int> Orientation_filter) {
        const int size=or_boxes.size();
        int rounds=ceil(float(size)/MAX_BATCH_SIZE);
        int index=-1;
        for(int round=0;round<rounds;round++){
            int curr_batch= MAX_BATCH_SIZE;
            ...
            net_.Reshape(data_shape);
            net_.Forward(data_map);
            const auto *ori_cls_data = net_.GetBlobDataByName<float>(prob_updown_);
            const auto *orient_data = net_.GetBlobDataByName<float>(orient_value_);
            for(int iter=0;iter<curr_batch;iter++){
                while(Orientation_filter[++index]!=1);
                const auto* cls_ptr = ori_cls_data+iter*cls_dim_;//2 categories
                float orient_angle = orient_data[iter];
                const BoxF box = or_boxes[round*MAX_BATCH_SIZE+iter];
                int label;float conf;
                OrientationRegression::Argmax(cls_ptr,cls_dim_,label,conf);
                if (label) {
                    orient_angle = (M_PI - orient_angle)/M_PI*180;
                } else if (orient_angle<0) {
                    orient_angle = (2*M_PI + orient_angle)/M_PI*180;
                }
                Orientation[index] = orient_angle;
            }
        }
    }

    void OrientationRegression::Argmax(const float* data,const int num,int& idx,float& conf){
        conf=data[0];
        idx=0;
        for (int iter = 1; iter < num; iter++) {
            if(data[iter]>conf){
