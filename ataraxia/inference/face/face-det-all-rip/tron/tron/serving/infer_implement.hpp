...
#define MAX_BATCH_SIZE (16)
...
#if defined(USE_OpenCV)
        bool AreaCompare (BoxF i,BoxF j);
    ...
    public:
        ...
        void Predict(const cv::Mat &im_mat, const VecBoxF &face_boxes,
                                     ...
                                     std::vector<int>& Qalabels, int min_face);
        
        void Release();
        
    private:
        void Process(const VecFloat &in_data, const VecBoxF &qa_boxes, std::vector<std::vector<float>> & Qaprob,
                                     std::vector<int>& Qalabels);
        
        void Argmax(const float* data,const int num,int& idx,float& conf);
        
        void GetQualityProb(std::vector<float>& probs,const float* cls_ptr);
        ...
        int cls_dim_;
        // int ori_dim_;
        std::string prob_quality_, prob_orient_;
    };

    class OrientationRegression {
    public:
        OrientationRegression() = default;
        ~OrientationRegression() { Release(); }
        
        void Setup(const tron::MetaNetParam &meta_net_param,
                   const VecInt &in_shape,int gpu_id);
        
        void Predict(const cv::Mat &im_mat, const VecBoxF &face_boxes,
                                     std::vector<float>& Orientation, std::vector<int> Orientation_filter, int min_face);
        
        void Release();
        
    private:
        void Process(const VecFloat &in_data, const VecBoxF &or_boxes, std::vector<float>& Orientation, std::vector<int> Orientation_filter);
        ...
        int cls_dim_;
        int ori_dim_;
        std::string prob_updown_, orient_value_;
    };
