...
int main(int argc, char const *argv[]) {
    ...
    std::string model_fd_file("data/models/refinedet_v0.0.3.tronmodel");
    std::string model_quality_file("data/models/quality_v0.0.3.tronmodel");
    std::string model_or_file("data/models/orientation_v0.0.3.tronmodel");
    ...
    auto *model_or_fp = fopen(model_or_file.c_str(), "rb");
    fseek(model_or_fp, 0, SEEK_END);
    auto model_or_size = ftell(model_or_fp);
    rewind(model_or_fp);
    std::vector<char> model_or_data(model_or_size, 0);
    fread(model_or_data.data(), 1, model_or_size, model_or_fp);
    fclose(model_or_fp);
    ...
    // orientation
    model_files = create_params.add_model_files();
    model_files->set_name(model_or_file.c_str());
    model_files->set_body(model_or_data.data(), model_or_size);

    const std::string custom_params=R"({"gpu_id": 0,"const_use_quality": 1,"output_quality_score": 1, "blur_threshold": 0.98, "min_face": 50, "rip_filter_type": 1})";
