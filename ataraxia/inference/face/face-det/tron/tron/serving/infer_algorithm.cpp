...
struct TronInferHandle {
    ...
    float blur_threshold=0.95;
    float quality_threshold=0;
    bool output_pts5=false;
...
int TronProcess(const cv::Mat &im_mat, TronInferHandle *handle,
    ...
    if (handle->fd_method_ == nullptr) {
        LOG(ERROR) << "Tron is uninitialized";
    ...
    if(handle->use_quality&&Gboxes[0].size()>0){
        if (handle->qa_method_ == nullptr){
            LOG(FATAL) << "Tron quality is uninitialized";
            return tron_status_method_nullptr;
        }
        handle->qa_method_->Predict(im_mat,Gboxes[0],Gpoints,Qalabels);
    }
    
    boxes=Gboxes[0];
    
    for (int iter=0;iter<boxes.size();iter++){
        if(!handle->use_quality){//only facedetection
            ...
            if(Qalabels[iter]!=1){//not neg face
                ...
                if(!handle->output_pts5){
                    rect.category=Qalabels[iter];
                    for(int i=0;i<5;++i){
                        rect.pts5[2*i]=-1; rect.pts5[2*i+1]=-1;
                    }
                }else{
                    const auto& pts5=Gpoints[iter];
                    rect.category=Qalabels[iter];
                    for(int i=0;i<5;++i){
                        rect.pts5[2*i]=pts5[i].x; rect.pts5[2*i+1]=pts5[i].y;
                    }
                }
                output->objects.push_back(rect);
            }//if(Qalabels[iter]!=1)
        }
    }//for (int iter=0;iter<Qalabels.size();iter++)
    // done
...
void *createNet(void *params, const int params_size, int *code, char **err) {
    ...
    if(document.HasMember("use_quality")) {
        handle_->use_quality = Tron::get_int(document, "use_quality", 1);
    }
    if(document.HasMember("blur_threshold")) {
        handle_->blur_threshold = Tron::get_float(document, "blur_threshold",0.95);
    }
    if(document.HasMember("output_pts5")) {
        handle_->output_pts5 = Tron::get_int(document, "output_pts5", 0);
    }
    
    LOG(INFO)<<"gpu: "<<handle_->gpu_id_
    <<",use_quality= "<<handle_->use_quality
    <<",blur_threshold="<< handle_->blur_threshold
    <<",output_pts5="<<handle_->output_pts5;
    
    handle_->fd_method_ = new Tron::ShadowDetectionRefineDet();
    handle_->fd_method_->Setup(meta_net_param_fd, {1}, handle_->gpu_id_);
    
    if(handle_->use_quality){
        ...
        if (req_data.has_body() && !req_data.body().empty()) {
            ...
            if (!im_mat.empty()) {
                ...
                if (status == tron_status_success) {
                    ...
                    const auto &result_json_str = Tron::get_detect_json(ptr->use_quality,ptr->output_pts5,detection_output);
                    res->set_code(status);
