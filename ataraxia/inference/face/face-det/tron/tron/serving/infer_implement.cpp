...
namespace Tron {
    ...
    void QualityEvalutation::Predict(const cv::Mat &im_mat, const VecBoxF &face_boxes,
        ...
        Gpoints.resize(face_boxes.size());
        for(int n=0;n<face_boxes.size();n++){
            Gpoints[n].resize(pts5_dim_);
        }
        Qalabels.resize(face_boxes.size(),-1);
        VecBoxF qa_boxes;
        float scale=0.0143;//expend face box
        int index=-1;
        ...
        if(qa_boxes.size()>0) {
            ...
            Process(in_data_, qa_boxes,Gpoints,Qalabels);
        }
    ...
    void QualityEvalutation::Process(const VecFloat &in_data, const VecBoxF &qa_boxes, std::vector<Tron::VecPointF>& Gpoints,
                                     std::vector<int>& Qalabels) {
        ...
        for(int round=0;round<rounds;round++){
            ...
            net_.Forward(data_map);
            const auto *cls_data  = net_.GetBlobDataByName<float>(fc6_cls_str_);
            const auto *pts5_data = net_.GetBlobDataByName<float>(fc6_pts5_str_);
            for(int iter=0;iter<curr_batch;iter++){
                while(Qalabels[++index]!=-1);
                const auto* cls_ptr = cls_data+iter*cls_dim_;//5 category
                const auto* pts5_ptr = pts5_data+iter*pts5_dim_;//5*2 [x,y]
                ...
                switch(label){
                    case 0 : //quality
                        Qalabels[index] = 0;
                        GetPts5(box,Gpoints[index],pts5_ptr);
                        break;
                    case 1 : //neg
                        Qalabels[index] = 1;
                        break;
