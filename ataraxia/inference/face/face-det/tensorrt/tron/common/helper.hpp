    ...
    if (create_params.model_files_size() > 0) {
      ...
      for(int i = 0;i<create_params.model_files_size();i++){
        ...
        if (model_file.has_body()) {
          engin_size[i] = model_file.body().size();
          const auto *body_data = model_file.body().data();
          engin.at(i).resize(engin_size[i]);
          memcpy((void*)engin.at(i).data(),body_data,engin_size[i]);
          LOG(INFO) <<model_file.name() << " Model file's body size: " << engin_size[i];
