    ...
    for tron_op in net_param.op:
        ...
        for caffe_layer in caffe_model.layer:
            if caffe_layer.name == op_name:
                assert len(tron_op.blobs) == len(caffe_layer.blobs)
                for n in range(0, len(tron_op.blobs)):
                    tron_blob = tron_op.blobs[n]
    ...
    if num_mean > 0 or num_scale > 0:
        for input_name in tron_inputs:
            if 'data' in input_name:
                tron_net.add_data('data_transform_' + input_name, [input_name], [input_name], mean_value, scale_value)
...
def convert_bias(caffe_layer, tron_net):
    ...
    tron_net.add_bias(layer_name, bottom_names, top_names, axis, num_axes)
...
def caffe2tron(model_root, meta_net_info, copy_params=False):
    ...
    for n in range(0, len(caffe_deploys)):
        ...
        tron_net.set_net(n)
        tron_net.set_net_name(caffe_deploy.name)
        tron_net.set_net_num_class(net_info['num_class'])
        tron_net.set_net_arg(net_info['arg'])
        tron_net.set_net_out_blob(net_info['out_blob'])
        ...
        if copy_params:
            copy_weights(caffe_models[n], tron_net)
