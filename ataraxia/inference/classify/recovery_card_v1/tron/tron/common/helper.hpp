...
inline std::string get_classify_json(
  ...
  */
  ...
  if (final_index == -1){
    final_index = 5; // normal
    final_score = scores[top_index[0]];
  }
  */
  std::string output_label_str = classification_output.labels[top_index[0]];
  int split_index = output_label_str.find("_");
  std::string index = output_label_str.substr(0,split_index);
  std::string class_name = output_label_str.substr(split_index+1);
  final_score = scores[top_index[0]];
  if (final_score > threshold){
     final_index = std::stoi(index);
  ...
  j_class.AddMember("index", Value(final_index), alloc);
  j_class.AddMember("score", Value(final_score), alloc);
  j_class.AddMember("class", Value(StringRef(final_class.c_str())), alloc);
