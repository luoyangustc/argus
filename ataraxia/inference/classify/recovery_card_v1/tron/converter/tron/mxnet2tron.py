...
def convert_binary(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    if json_op == '_plus_scalar':
        ...
        raise ValueError('Unsupported binary type', act_type)

    scalar = parse_param(json_attr, 'scalar', 's_f', 1)

    tron_net.add_binary(json_name, bottom_names, [json_name], operation_type, scalar)
...
def convert_leakyrelu(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_activate(json_name, bottom_names, [json_name], act_type, slope=slope)
    ...
    for n in range(0, len(mxnet_symbols)):
        ...
        for index, json_node in enumerate(mxnet_nodes):
            ...
            if json_op == 'Activation':
                ...
                convert_binary(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == 'Concat':
                ...
                convert_leakyrelu(mxnet_nodes, index, param_dict, tron_net)
