...
class Operator {
 public:
  explicit Operator(const tron::OpParam &op_param, Workspace *ws);
  virtual ~Operator();
...
Operator *CreateOperator(const tron::OpParam &op_param, Workspace *ws);

SHADOW_DECLARE_REGISTRY(OperatorRegistry, Operator, const tron::OpParam &,
                        Workspace *);
