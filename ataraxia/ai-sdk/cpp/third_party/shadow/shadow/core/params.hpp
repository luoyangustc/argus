...
class ArgumentHelper {
 public:
  ...
  explicit ArgumentHelper(const tron::NetParam &def);
  explicit ArgumentHelper(const tron::OpParam &def);
 ...
 private:
  std::map<std::string, tron::Argument> arg_map_;
};

#define INSTANTIATE_SET_SINGLE_ARGUMENT(T, fieldname)                    \
  static void set_##fieldname(tron::OpParam *op_param,                   \
