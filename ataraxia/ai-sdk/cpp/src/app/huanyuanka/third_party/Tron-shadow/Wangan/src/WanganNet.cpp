...
WanganNet::WanganNet(vector<vector<float>> &preParam, InterMethod method)
{
...
void WanganNet::resizeModel(int modelNum_)
{
...
ShadowStatus WanganNet::init(const int gpuID, const std::vector<std::vector<char>> data,const std::vector<int> size)
{
...
ShadowStatus WanganNet::allocateMemory()
{   
...
void WanganNet::processImageGPU(const vector<cv::Mat> &imgs, int model_id)
{
...
void **WanganNet::getBuffers(int model_id)
{
...
ShadowStatus WanganNet::predict(const std::vector<cv::Mat> &imgs, const std::vector<std::string> &outputlayer, std::vector<std::vector<float> > &results,int enginIndex)
{
    ...
    const char *detoutputlayer = (outputIndex != -1) ? models[outputIndex].engine->getBindingName(3) : "";
...
ShadowStatus WanganNet::destroy()
{
    for(size_t model_id = 0; model_id < models.size(); model_id++)
    {
        Model &model = models[model_id];
