...
class WanganNet : public Net
{
public:
  WanganNet(vector<vector<float>> &preParam, InterMethod method = bilinear);
...
private:
  ...
  int detIndex = -1; //index of det model 
  int outputIndex = -1; //index of output layer(last layer of det model)
