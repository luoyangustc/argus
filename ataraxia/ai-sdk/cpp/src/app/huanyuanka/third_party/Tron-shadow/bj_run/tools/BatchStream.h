...
public:
    ...
    {
        FILE* file = fopen(locateFile(std::string("batch0")).c_str(), "rb");
        int d[4];
...
private:
    ...
    bool update()
    {
        std::string bathName = std::string("batch") + std::to_string(mFileCount++);
        std::string inputFileName = locateFile(bathName);
    ...
    int mBatchSize{ 0 }; //batchsize
    int mMaxBatches{ 0 };//max batch size
    int mBatchCount{ 0 };
