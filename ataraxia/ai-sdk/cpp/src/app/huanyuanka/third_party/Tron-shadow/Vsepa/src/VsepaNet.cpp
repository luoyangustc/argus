...
//获取输入输出相关信息，并分配显存，
ShadowStatus VsepaNet::allocateMemory()
{
...
//初始化网络，
ShadowStatus VsepaNet::init(const int gpuID, void *data, int size)
{
    if (data == NULL || size == 0)
...
void VsepaNet::processImageGPU(const vector<cv::Mat> &imgs)
{
...
ShadowStatus VsepaNet::predict(const vector<cv::Mat> &imgs, const std::vector<std::string> &attributes, std::vector<std::string> &results)
{
...
void VsepaNet::dealResult()
{
    ...
    for (size_t k = 0; k < batchSize; k++)
    {
        ...
        {
            std::cout << outputData[k * outputSize + i] << " ";
        }
    }
    gProfiler.printLayerTimes();
}

void VsepaNet::printTime()
{
