...
struct JsonReaderStackItem {
  ...
  JsonReaderStackItem(const Value* value0, State state0) : value(value0), state(state0), index() {}
...
JsonReader& JsonReader::operator&(float& d) {
  if (!mError) {
    if (CURRENT.IsNumber()) {
      d = CURRENT.GetFloat();
      Next();
...
JsonWriter& JsonWriter::operator&(float& d) {
  WRITER->Double(static_cast<double>(d));
  return *this;
}
