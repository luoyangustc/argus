...
template <typename Dtype>
float IoU(const Box<Dtype> &box_a, const Box<Dtype> &box_b, bool is_iom) {
  if (is_iom) {
    return Intersection(box_a, box_b) / std::min(Size(box_a), Size(box_b));
  } else {
...
std::vector<Box<Dtype>> NMS(const std::vector<Box<Dtype>> &boxes,
                            float iou_threshold, bool is_iom) {
  ...
  for (size_t i = 0; i < all_boxes.size(); ++i) {
    ...
    for (size_t j = i + 1; j < all_boxes.size(); ++j) {
      ...
      if (IoU(box_i, box_j, is_iom) > iou_threshold) {
        box_j.label = -1;
      }
    }
  }
...
std::vector<Box<Dtype>> NMS(const std::vector<std::vector<Box<Dtype>>> &Gboxes,
  ...
  return NMS(all_boxes, iou_threshold, is_iom);
}
...
template float IoU(const BoxI &box_a, const BoxI &box_b, bool is_iom);
template float IoU(const BoxF &box_a, const BoxF &box_b, bool is_iom);

template VecBoxI NMS(const VecBoxI &boxes, float iou_threshold, bool is_iom);
template VecBoxF NMS(const VecBoxF &boxes, float iou_threshold, bool is_iom);
