...
class ForwardServer(BaseForwardServer):
    def __init__(self, app_name, batch_size, lock, cfg):
        ...
        coarse_deploy = str(cfg['model_files']['coarse_deploy.prototxt'])
        coarse_weight = str(cfg['model_files']['coarse_weight.caffemodel'])
        ...
        new_coarse_deploy = change_deploy(coarse_deploy, batch_size)
        new_det_deploy = change_deploy(det_deploy, batch_size)
        self.net_fine = caffe.Net(new_fine_deploy, fine_weight, caffe.TEST)
        self.net_coarse = caffe.Net(new_coarse_deploy, coarse_weight,
                                    caffe.TEST)
