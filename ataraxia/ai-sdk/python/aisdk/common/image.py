...
from .error import ErrorFileNotExist, ErrorCV2ImageRead, ErrorInvalidPTS, ErrorImageTooLarge, ErrorImageTooSmall, ErrorImageNdim
assert cv2.__version__[0] == '4'


def load_imagev2(body, image_desc='image from body'):
    '''
    ...
    if len(body) > 10 * 1024 * 1024:
        raise ErrorImageTooLarge(image_desc)
    if body is None or len(body) == 0:
        raise ErrorCV2ImageRead(image_desc)
    image = cv2.imdecode(np.asarray(bytearray(body), dtype=np.uint8), 1)
    if image is None:
        raise ErrorCV2ImageRead(image_desc)
    if image.shape[0] <= 32 or image.shape[1] <= 32:
        raise ErrorImageTooSmall(image_desc)
    if image.ndim != 3:
        raise ErrorImageNdim
...
def load_image(image_file, body=None):
    ...
    if image.shape[0] <= 32 or image.shape[1] <= 32:
        raise ErrorImageTooSmall(image_file)
    if image.ndim != 3:
        raise ErrorImageNdim
    if image.shape[0] > 4999 or image.shape[1] > 4999:
        raise ErrorImageTooLarge(image_file)
...
def check_valid_pts(pts):
    return (len(pts) == 4 and pts[1][0] > pts[0][0] and pts[3][1] > pts[0][1]
            and pts[1][0] - pts[0][0] == pts[2][0] - pts[3][0]
            and pts[2][1] - pts[1][1] == pts[3][1] - pts[0][1])


def load_image_roi_by_4pts(image_file,
                           pts,
                           roi_scale=1.0,
                           output_square=0,
                           body=None):
    ...
    roi_img = load_image_roi(
        image_file, roi, roi_scale, output_square, body=body)
