...
def copy_weights(caffe_model, tron_net):
    net_param = tron_net.net_param
    for tron_op in net_param.op:
        op_name = tron_op.name
        for caffe_layer in caffe_model.layer:
            if caffe_layer.name == op_name:
                for n, caffe_blob in enumerate(caffe_layer.blobs):
                    ...
                    tron_blob = net_param.blob.add()
                    tron_blob.name = op_name + '_weights:{}'.format(n)
                    tron_blob.shape.extend(blob_shape)
                    tron_blob.data_f.extend(caffe_blob.data)
                    tron_op.bottom.append(tron_blob.name)
                break


def convert_input(caffe_deploy, net_info, tron_net):
    start_layer = 0
    tron_inputs = []
    tron_shapes = []

    if len(caffe_deploy.input) > 0:
        tron_inputs.extend(caffe_deploy.input)
        if len(net_info['input_shape']) == 0:
            if len(caffe_deploy.input_shape) > 0:
                for caffe_shape in caffe_deploy.input_shape:
                    tron_shapes.append(caffe_shape.dim)
            elif len(caffe_deploy.input_dim) > 0:
                num_dim = len(caffe_deploy.input_dim)
                num_shape = int(num_dim / 4)
                for i in range(0, num_shape):
                    tron_shapes.append(caffe_deploy.input_dim[4*i : 4*(i+1)])
        else:
            tron_shapes = net_info['input_shape']
        start_layer = 0
    elif caffe_deploy.layer[0].type == 'Input':
        caffe_input_layer = caffe_deploy.layer[0]
        tron_inputs.extend(caffe_input_layer.top)
        if len(net_info['input_shape']) == 0:
    ...
    assert len(tron_inputs) == len(tron_shapes)
    tron_net.add_input('input', [], tron_inputs, tron_shapes)
    ...
    if num_mean > 0 or num_scale > 0:
        ...
        for input_name in tron_inputs:
            if 'data' in input_name:
                tron_net.add_scale(input_name, [input_name], [input_name], 1, False, False, scale_value, mean_value)

    return start_layer


def convert_activate(caffe_layer, tron_net):
    ...
    tron_net.add_activate(layer_name, bottom_names, top_names, act_type)


def convert_axpy(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    layer_type = caffe_layer.type
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top

    tron_net.add_axpy(layer_name, bottom_names, top_names)


def convert_batch_norm(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_batch_norm(layer_name, bottom_names, top_names, use_global_stats, eps)


def convert_bias(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_scale(layer_name, bottom_names, top_names, axis, False, True)


def convert_concat(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_concat(layer_name, bottom_names, top_names, axis)


def convert_connected(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_connected(layer_name, bottom_names, top_names, num_output, bias_term, transpose)


def convert_conv(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_conv(layer_name, bottom_names, top_names, num_output, kernel_size, stride, pad, dilation, bias_term, group)


def convert_deconv(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_deconv(layer_name, bottom_names, top_names, num_output, kernel_size, stride, pad, dilation, bias_term, group)


def convert_eltwise(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_eltwise(layer_name, bottom_names, top_names, eltwise_op, coeff)


def convert_flatten(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_flatten(layer_name, bottom_names, top_names, axis, end_axis)


def convert_lrn(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_lrn(layer_name, bottom_names, top_names, local_size, alpha, beta, norm_region, k)


def convert_normalize(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_normalize(layer_name, bottom_names, top_names, across_spatial, channel_shared)


def convert_permute(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_permute(layer_name, bottom_names, top_names, order)


def convert_pooling(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_pooling(layer_name, bottom_names, top_names, pool, kernel_size, stride, pad, global_pooling)


def convert_prior_box(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_prior_box(layer_name, bottom_names, top_names, min_size, max_size, aspect_ratio, flip, clip, variance, step, offset)


def convert_python(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    if caffe_layer.HasField('python_param'):
        ...
        if caffe_param.HasField('layer'):
            if caffe_param.layer == 'ProposalLayer':
                ...
                tron_net.add_proposal(layer_name, bottom_names, top_names)
            else:
...
def convert_psroi_pooling(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_psroi_pooling(layer_name, bottom_names, top_names, output_dim, group_size, spatial_scale)


def convert_reshape(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_reshape(layer_name, bottom_names, top_names, shape, axis, num_axes)


def convert_roi_pooling(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_roi_pooling(layer_name, bottom_names, top_names, pooled_h, pooled_w, spatial_scale)


def convert_scale(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_scale(layer_name, bottom_names, top_names, axis, num_axes, bias_term)


def convert_slice(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_slice(layer_name, bottom_names, top_names, axis, slice_point)


def convert_softmax(caffe_layer, tron_net):
    layer_name = caffe_layer.name
    bottom_names = caffe_layer.bottom
    top_names = caffe_layer.top
    ...
    tron_net.add_softmax(layer_name, bottom_names, top_names, axis)


def caffe2tron(model_root, meta_net_info, copy_params=False):
    tron_net = Tron(meta_net_info['save_name'], meta_net_info)
    ...
    for n, model_name in enumerate(meta_net_info['model_name']):
        ...
        tron_net.set_net(n)
        tron_net.set_net_name(meta_net_info['model_name'][n])
        tron_net.set_net_arg(net_info['arg'])

        start_layer = convert_input(caffe_deploy, net_info, tron_net)
        ...
        for index in range(start_layer, len(caffe_deploy.layer)):
            ...
            if layer_type == 'PReLU' or layer_type == 'ReLU' or layer_type == 'Sigmoid':
                convert_activate(caffe_layer, tron_net)
            elif layer_type == 'Axpy':
                convert_axpy(caffe_layer, tron_net)
            elif layer_type == 'BatchNorm':
                convert_batch_norm(caffe_layer, tron_net)
            elif layer_type == 'Bias':
                convert_bias(caffe_layer, tron_net)
            elif layer_type == 'Concat':
                convert_concat(caffe_layer, tron_net)
            elif layer_type == 'InnerProduct':
                convert_connected(caffe_layer, tron_net)
            elif layer_type == 'Convolution' or layer_type == 'DepthwiseConvolution':
                convert_conv(caffe_layer, tron_net)
            elif layer_type == 'Deconvolution':
                convert_deconv(caffe_layer, tron_net)
            elif layer_type == 'Eltwise':
                convert_eltwise(caffe_layer, tron_net)
            elif layer_type == 'Flatten':
                convert_flatten(caffe_layer, tron_net)
            elif layer_type == 'LRN':
                convert_lrn(caffe_layer, tron_net)
            elif layer_type == 'Normalize':
                convert_normalize(caffe_layer, tron_net)
            elif layer_type == 'Permute':
                convert_permute(caffe_layer, tron_net)
            elif layer_type == 'Pooling':
                convert_pooling(caffe_layer, tron_net)
            elif layer_type == 'PriorBox':
                convert_prior_box(caffe_layer, tron_net)
            elif layer_type == 'PSROIPooling':
                convert_psroi_pooling(caffe_layer, tron_net)
            elif layer_type == 'Python':
                convert_python(caffe_layer, tron_net)
            elif layer_type == 'Reshape':
                convert_reshape(caffe_layer, tron_net)
            elif layer_type == 'ROIPooling':
                convert_roi_pooling(caffe_layer, tron_net)
            elif layer_type == 'Scale':
                convert_scale(caffe_layer, tron_net)
            elif layer_type == 'Slice':
                convert_slice(caffe_layer, tron_net)
            elif layer_type == 'Softmax':
                convert_softmax(caffe_layer, tron_net)
            else:
        ...
        if copy_params:
            ...
            copy_weights(caffe_model, tron_net)
