...
def copy_weights(arg_params, aux_params, param_dict, tron_net):
    ...
    for tron_op in net_param.op:
        ...
        for n, param_name in enumerate(param_dict[op_name]):
            ...
            tron_blob.shape.extend(param_data.shape)
            tron_blob.data_f.extend(param_data.flatten())
...
def convert_batch_norm(mxnet_nodes, index, param_dict, arg_params, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_scale(json_name + '_scale', [json_name], [json_name], 1)
...
def convert_deform_conv(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_deform_conv(json_name, bottom_names, [json_name], num_output, kernel_size[0], stride[0], pad[0], dilate[0], bias_term, group, deform_group)


def convert_deform_psroi_pooling(mxnet_nodes, index, param_dict, tron_net):
    json_node = mxnet_nodes[index]
    json_name = json_node['name']
    json_inputs = json_node['inputs']
    if params_str in json_node:
    ...
    tron_net.add_deform_psroi_pooling(json_name, bottom_names, [json_name], output_dim, group_size, pooled_size, part_size, sample_per_part, spatial_scale, trans_std, no_trans)
...
def convert_pooling(mxnet_nodes, index, param_dict, tron_net):
    ...
    tron_net.add_pooling(json_name, bottom_names, [json_name], pool, kernel_size, stride, pad, global_pooling, full_pooling)
    ...
    for n, model_name in enumerate(meta_net_info['model_name']):
        ...
        for index, json_node in enumerate(mxnet_nodes):
            ...
            if json_op == 'null':
                ...
                convert_batch_norm(mxnet_nodes, index, param_dict, arg_params, tron_net)
                ...
                convert_deform_conv(mxnet_nodes, index, param_dict, tron_net)
            elif json_op == '_contrib_DeformablePSROIPooling':
                convert_deform_psroi_pooling(mxnet_nodes, index, param_dict, tron_net)
        ...
        if copy_params:
            copy_weights(arg_params, aux_params, param_dict, tron_net)
