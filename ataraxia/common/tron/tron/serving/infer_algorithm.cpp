...
struct TronInferHandle {
  ...
  std::shared_ptr<Classification> method_ = nullptr;
};
...
int TronProcess(const cv::Mat &im_mat, TronInferHandle *handle,
  ...
  std::vector<std::map<std::string, VecFloat>> Gscores;
  VecRectF rois{RectF(0, 0, im_w, im_h)};
...
void *createNet(void *params, const int params_size, int *code, char **err) {
  ...
  bool success =
      parse_create_params(params, params_size, &custom_params, &meta_net_param);

  if (!success) {
    *code = tron_status_parse_model_error;
    *err = const_cast<char *>(get_status_message(*code));
    return nullptr;
  }

  // Parse some useful params
  const auto &document = get_document(custom_params);
  if (document.HasMember("gpu_id")) {
    handle_->gpu_id_ = get_int(document, "gpu_id", 0);
  }
  if (document.HasMember("threshold")) {
    handle_->threshold_ = get_float(document, "threshold", -1);
  }
  if (document.HasMember("top_k")) {
    handle_->top_k_ = get_int(document, "top_k", -1);
  }

  // Initial network
  handle_->method_ = std::make_shared<Classification>();
  handle_->method_->Setup(meta_net_param, handle_->gpu_id_);
    ...
    if (req_data.has_body() && !req_data.body().empty()) {
      ...
      if (!im_mat.empty()) {
        ...
        if (status == tron_status_success) {
          ...
          const auto &result_json_str = get_classify_json(
              classification_output, ptr->top_k_, ptr->threshold_);
          res->set_code(status);
          res->set_message(get_status_message(status));
