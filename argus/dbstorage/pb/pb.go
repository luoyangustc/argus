...
// Increment current value & success value
func (pb *ProgressBar) IncrementSuccess() int {
	atomic.AddInt64(&pb.success, 1)
	return pb.Add(1)
}

// Increment current value & success value
func (pb *ProgressBar) IncrementFailure() int {
	atomic.AddInt64(&pb.fail, 1)
	return pb.Add(1)
}
...
func (pb *ProgressBar) Finish() {
	...
	pb.finishOnce.Do(func() {
		close(pb.finish)
		pb.write(atomic.LoadInt64(&pb.Total), atomic.LoadInt64(&pb.current), atomic.LoadInt64(&pb.success), atomic.LoadInt64(&pb.fail))
		pb.mu.Lock()
		defer pb.mu.Unlock()
		switch {
		case pb.Output != nil:
...
func (pb *ProgressBar) write(total, current, success, fail int64) {
	width := pb.GetWidth()
	...
	if pb.ShowCounters {
		...
		if total > 0 {
			...
			successS := Format(success).To(pb.Units).Width(pb.UnitsWidth)
			failS := Format(fail).To(pb.Units).Width(pb.UnitsWidth)
			countersBox = fmt.Sprintf(" Process: %s/%s (success: %s, fail: %s) ", current, totalS, successS, failS)
		} else {
	...
	if pb.ShowElapsedTime {
		timeSpentBox = fmt.Sprintf("Time spent: %s ", (fromStart/time.Second)*time.Second)
	}

	select {
	case <-pb.finish:
		if pb.ShowFinalTime {
			var left time.Duration = (fromStart / time.Second) * time.Second
	...
	default:
		if pb.ShowTimeLeft && currentFromStart > 0 {
			...
			if left > 0 {
				...
				timeLeftBox = fmt.Sprintf(" Time left: %s", timeLeft)
			}
		}
	}
	...
	if pb.ShowSpeed && currentFromStart > 0 {
		...
		speedBox = " Speed: " + Format(int64(speed)).To(pb.Units).Width(pb.UnitsWidth).PerSec().String()
	}
...
func (pb *ProgressBar) Update() {
	c := atomic.LoadInt64(&pb.current)
	s := atomic.LoadInt64(&pb.success)
	f := atomic.LoadInt64(&pb.fail)
