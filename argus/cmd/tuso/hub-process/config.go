...
type Config struct {
	...
	FeatureApi     config.StaticConfigValue `json:"feature_api"`
	ConcurrencyNum config.WatchConfigValue  `json:"concurrency_num"`
}
...
func InitConfig(ctx context.Context, conf *Config) error {
	...
	{
		conf.ConcurrencyNum = config.NewWatchEtcdValue(
			ctx, client, "/ava/argus/tuso/image_feature_concurrency_num",
