...
func main() {
	...
	cconf.Init("f", "face_group", "face_group.conf")
	...
	al, logf, err := jsonlog.Open("FG", &conf.FileConfig.AuditLog, nil)
	if err != nil {
	...
	var (
		...
		service     *FACEG.FaceGroupService
		sts         STS.Client
	)
	// 创建 STS.Client
	{
	...
	}
	{
		...
		service, update = FACEG.NewFaceGroupService(_config, sts, m, featureAPIs)
