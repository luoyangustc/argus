...
func main() {
	...
	cconf.Init("f", "censor-image-worker", "censor-image-worker.conf")
	...
	mux.HandleFunc("GET /health", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte("ok censor-image-worker"))
	})
	...
	var ciConfig workers.ImageConfig
	if vv, err := conf.Biz.Value(ctx); err != nil {
		xl.Fatalf("load biz config failed. %v", err)
	} else {
		ciConfig = vv.(workers.ImageConfig)
	}

	xl.Info("load conf", config.DumpJsonConfig(ciConfig), config.DumpJsonConfig(conf))

	{
		...
		ciWorker := workers.NewImageWorker(ciConfig)
		_, _ = job.NewAsyncS(
			...
			func(ctx context.Context, req []byte) ([]byte, error) {
				return ciWorker.Do(ctx, job.NewTask(req))
			},
		)
	}
