...
func main() {
	...
	cconf.Init("f", "bucket-censor-master", "bucket-censor-master.conf")
	...
	al, logf, err := jsonlog.Open("BS", &conf.FileConfig.AuditLog, nil)
	if err != nil {
	...
	var scanConfig bucket_censor.ScanConfig
	if vv, err := conf.ScanConfig.Value(ctx); err != nil {
		xl.Fatalf("load scan config failed. %v", err)
	} else {
		scanConfig = vv.(bucket_censor.ScanConfig)
	}

	xl.Info("load conf", config.DumpJsonConfig(scanConfig), config.DumpJsonConfig(mqConfig))
	...
	var (
		...
		mq, _  = job.NewMgoAsMQ(mqConfig, JOB_CMD)
		scan   = bucket_censor.NewScanNode(scanConfig)
		client = job.NewAsyncC(
			...
			func() string {
				if node.TopicDefault != "" {
					return node.TopicDefault
				}
				return "first_argus_job_" + JOB_CMD
			}(),
			node.TopicOptions,
			func() string {
	...
	var master = job.NewMasterNode(node, mq, client, scan)
	go master.Run()

	xl.Infof("Listen: %s", config.GetString(ctx, conf.HTTPPort, "80"))
