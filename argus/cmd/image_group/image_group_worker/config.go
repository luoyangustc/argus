...
type Config struct {
	...
	Worker  config.StaticConfigValue
...
func InitConfig(ctx context.Context, conf *Config) error {
	...
	}
	{
		conf.Worker = config.NewMultiStaticConfigValue(
			...
			config.NewStaticEtcdValue(
				etcd.NewKV(client), "/ava/argus/image_group/worker",
	...
	}
	{
		conf.Storage = config.NewMultiStaticConfigValue(
			...
			config.NewStaticEtcdValue(
				etcd.NewKV(client), "/ava/argus/image_group/cache",
				func(bs []byte) (interface{}, error) {
	...
	}
	{
		conf.Fetch = config.NewMultiStaticConfigValue(
			...
			config.NewStaticEtcdValue(
				etcd.NewKV(client), "/ava/argus/image_group/fetch",
				func(bs []byte) (interface{}, error) {
					var v struct {
						URL string `json:"url"`
					}
