...
func main() {
	...
	cconf.Init("f", "image_group_worker", "image_group_worker.conf")
	...
	{
		v, _ := conf.Worker.Value(ctx)
		wConfig := v.(WorkerConfig)
		xl.Info("worker conf", config.DumpJsonConfig(wConfig))
		if wConfig.GOGC != nil {
			debug.SetGCPercent(*wConfig.GOGC)
		}
	}

	al, logf, err := jsonlog.Open("IG_WORKER", &conf.FileConfig.AuditLog, nil)
	if err != nil {
	...
	mux.HandleFunc("GET /health", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte("ok image-group-worker"))
	})
