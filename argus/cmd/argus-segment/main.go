...
func main() {
	...
	// 加载配置
	{
		cconf.Init("f", "argus-segment", "argus-segment.conf")
	...
	var consumer eval.Consumer
	{
		...
		ccs := func() []sq.ConsumerConfig {
			...
			for _, c1 := range sqd {
				...
				c2.Topic = "first_argus_segment" // TODO
		...
		consumer, _ = segment.NewConsumer(seg, sts, ccs)
	...
	{
		// 审计日志
		al, logf, err := jsonlog.Open("ARGUS_SEGMENT", &conf.AuditLog, nil)
		if err != nil {
