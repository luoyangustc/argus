...
func NewHandler(rcvr reflect.Value, method reflect.Method) (http.Handler, error) {
	...
	if mtype.NumOut() != 0 || mtype.NumIn() != 3 {
		log.Debug("method", method.Name, "has wrong number arguments or return values:", mtype.NumIn(), mtype.NumOut())
		return nil, syscall.EINVAL
	}

	// First arg muste be http.ResponseWriter
	if wType := mtype.In(1); wType != typeOfHttpResponseWriter {
		log.Debug("method", method.Name, "first argument type not http.ResponseWriter:", wType)
		return nil, syscall.EINVAL
	}

	// Second arg must be *http.Request
	if reqType := mtype.In(2); reqType != typeOfHttpRequest {
		log.Debug("method", method.Name, "second arguement type not *http.Request:", reqType)
		return nil, syscall.EINVAL
	}
...
func prefixOf(name string) (prefix string, ok bool) {
	...
	if !Is(UPPER, rune(name[0])) {
		return
	}
	for i := 1; i < len(name); i++ {
		if !Is(LOWER, rune(name[i])) {
			return name[:i], true
		}
	}
	return
