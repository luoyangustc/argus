...
var (
	...
	RemoteExtraUserID = "X-Remote-Extra-User-ID"
)
...
func (r Client) Do(ctx Context, req *http.Request) (resp *http.Response, err error) {
	...
	if ruser, ok := ctx.Value(RemoteUser).(string); ok {
		req.Header.Set(RemoteUser, ruser)
	}

	if ruserid, ok := ctx.Value(RemoteExtraUserID).(string); ok {
		req.Header.Set(RemoteExtraUserID, ruserid)
	}
